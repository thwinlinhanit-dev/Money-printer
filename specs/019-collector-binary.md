...
- 2026-08-04: Watchdog runs as a Scheduled Task `MoneyPrinterCollectorsWatchdog` so collection survives reboot/logout. Registered non-elevated for the current user (`ops/watchdog_collectors.ps1 -RegisterTask`): an interactive task with a logon-only trigger, because standard users cannot create AtStartup-trigger tasks (access denied — verified 2026-08-04) and `-AsSystem` is unsuitable here (the watchdog runs `cargo build`, and rustup/cargo are installed per-user, so a SYSTEM context has no cargo on PATH). Settings: `MultipleInstances=IgnoreNew` (the boot+logon trigger pair collapses into a single watchdog), `ExecutionTimeLimit=3650d`, restart every 1 min on failure. Start manually with `Start-ScheduledTask MoneyPrinterCollectorsWatchdog`.
