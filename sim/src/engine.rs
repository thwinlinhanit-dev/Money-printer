...
struct SimCtx {
    ...
    rng: u64,
    /// Timers queued this turn: `(fire_at_ns, id)`. The engine drains these
    /// into `Backtester::pending_timers` after the strategy call returns.
...
pub struct Backtester {
    ...
    seq: u64,
    next_intent: u128,
    /// Event-time timer heap: `(fire_at_ns, TimerId)` sorted ascending.
    /// Populated by `Ctx::set_timer`; fired before every feature→strategy call
    /// so `on_timer` semantics match a live event loop (Major #5).
...
impl Backtester {
    ...
    fn on_event(&mut self, ev: &EventEnvelope) {
        ...
        // Fire any event-time timers that have elapsed (Major #5: on_timer is
        // wired, not dead trait surface). Sort ascending so earlier timers fire
        // first; call on_timer for each and enqueue its returned intents.
        self.pending_timers.sort_by_key(|&(t, _)| t);
        let now = ev.recv_ts_ns;
        // Collect timers due at `now` without requiring nightly drain_filter.
        let still: Vec<(i64, TimerId)> = self
            .pending_timers
            .iter()
            .copied()
            .filter(|&(fire_at, _)| fire_at > now)
            .collect();
        let fired: Vec<(i64, TimerId)> = self
            .pending_timers
            .iter()
            .copied()
            .filter(|&(fire_at, _)| fire_at <= now)
        ...
        for (_, timer_id) in fired {
            ...
            let intents = self.strat.on_timer(timer_id, &mut ctx);
            self.rng = ctx.rng;
            self.next_timer_id = ctx.next_timer;
            for timer in ctx.queued_timers {
    ...
    /// Returns `(price, qty, used_tape_fallback)`.
    fn try_fill_market(&mut self, symbol: SymbolId, now: i64) {
        ...
        for p in ready {
            ...
            match filled {
                Some((px, qty, tape_fallback)) if qty > 0.0 => {
                    // Major #8: tape fallback is untagged model degradation —
                    // tag it as optimistic_maker so G1 and metrics surface it.
                    self.execute_fill(
                        symbol,
                        p.venue,
                        p.side,
                        ...
                        p.intent_id,
                        Liquidity::Taker,
                        tape_fallback, // optimistic_tape → optimistic_maker bucket
                        now,
                    );
                    if qty < p.qty {
                        still.push(Pending {
    ...
    /// Returns `(price, qty, used_tape_fallback)`.
    fn fill_l1_market(&self, symbol: SymbolId, side: Side, qty: f64) -> Option<(f64, f64, bool)> {
    ...
    /// Tape fallback: fill at latest mark ± slip when the book is stale/absent
    /// (Major #8: tagged `true` so callers set the optimistic_maker flag).
    fn fallback_trade_price(
        ...
        Some((px, qty, true)) // tape_fallback = true → optimistic_tape
    }
    ...
    fn execute_fill(
        &mut self,
        symbol: SymbolId,
        _venue: Venue, // reserved: Fill will carry venue when OMS wires it (EXE-9)
        side: Side,
        price: f64,
        qty: f64,
        intent_id: mp_core::IntentId,
        ...
        self.log.record_fill_tagged(self.seq, &fill, optimistic_maker);

        // Major #5: call on_fill after accounting so strategies can respond
        // to their own fills (e.g. scale out, set a stop). Intents returned
        // go through the same risk gate → fill pipeline.
        // Build ctx fields first (avoids simultaneous &mut self + &mut self.strat).
        let mut ctx = SimCtx {
            now,
            equity: self.acct.equity(),
            positions: self.acct.positions(),
        ...
        let follow_intents = self.strat.on_fill(&fill, &mut ctx);
        self.rng = ctx.rng;
        self.next_timer_id = ctx.next_timer;
        for timer in ctx.queued_timers {
            self.pending_timers.push(timer);
