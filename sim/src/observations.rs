...
use mp_features::{FeatureUpdate, QualityTracker};
...
impl ObservationRecorder {
    ...
    /// strategy's quality tracker — determinism by construction).
    ...
    /// snapshot = the feature update(s) that caused the fire). Fires blocked
    ...
    pub fn record_intent(&mut self, intent: &OrderIntent, now_ns: i64, snapshot: &BTreeMap<String, f64>) {
        ...
        if let Some(obs) = engine.record(now_ns, intent.symbol, intent.venue, direction, snapshot.clone())
        {
