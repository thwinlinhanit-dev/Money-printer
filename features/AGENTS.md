...
- `src/corr.rs` — cross-asset correlation features (spec 048, COR-1..7): rolling Pearson r over daily returns from own recorded BTC/ETH + FRED/DeFiLlama/Coinalyze MacroPoints; `CorrRegimeFeature` regime classification (0=risk_on/1=neutral/2=risk_off via `[corr.regime]` thresholds); 10 pairs configured (BTC-ETH, BTC-Gold, BTC-SPX, ETH-SPX, BTC-DeFiLlama TVL/stablecoin/DEX, BTC-Coinalyze OI/funding/LSR); registered under `[corr]` as global tick features
- `src/climax_variants.rs` — 6 non-standard exhaustion/expansion pattern detectors (V1–V6): VolumeExhaustion (V1), MultiBarExhaustion (V2), SqueezeExpansion (V3), VolumeDivergence (V4), AbsorptionBar (V5), VolExpansion (V6); bar-only (SWG-2 compatible) BarFeatures registered under `[climax_variants]`; discovered from BTC daily analysis 2017–2026 complementing the standard climax detector (spec 004); 16 unit tests (cv_1..cv_16) + engine registration test
