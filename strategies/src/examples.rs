...
/// Small extension so the example can name a venue without extra plumbing. The
/// engine attaches venue context to updates in real use; here we default to
/// Bybit (fixtures are single-venue).
trait FeatureUpdateVenue {
    fn symbol_venue(&self) -> mp_core::Venue;
}
impl FeatureUpdateVenue for FeatureUpdate {
    fn symbol_venue(&self) -> mp_core::Venue {
        mp_core::Venue::Bybit
    }
}
