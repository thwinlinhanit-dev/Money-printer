...
- `src/bin/mp-ops.rs` — ops CLI (`compact`/`audit`/`scorecard`/`promote`/`band-accuracy-decay`/`telegram-flush [--wait]`/`telegram-stale`/`p1-webhook`)
...
- The watchdog supervises `mp-whale` (spec 028 Hyperliquid on-chain census) alongside the market collectors via the same heartbeat/staleness model (`collectors/whale_positions.toml`); its log (`{date}_hyperliquid_positions.log`) feeds the liq-est band research edge (spec 029) and is not a required gate recording
...
- P1 webhook egress (owner decision 2026-08-06, audit 08-04 #3/#9): WIRED but dead until credentials — `mp-ops p1-webhook --id --detail` POSTs the P1 dispatch JSON to `MP_OPS_P1_WEBHOOK` via curl (https accepted, TLS is the host's) and fails loudly (exit 2, "dead until credentials") when the env var is unset; no code change needed once creds are provisioned
