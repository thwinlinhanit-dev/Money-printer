...
pub fn audit_raw_log(path: &Path, config: &AuditConfig) -> RawLogAudit {
    ...
    let mut reader = match LogReader::open(path) {
        Ok(reader) => reader,
        Err(error) => {
            audit.findings.push(finding("unreadable_log", error.to_string()));
            return audit;
        }
    };
    ...
    loop {
        let event = match reader.next() {
            Some(Ok(event)) => event,
            Some(Err(error)) => {
                audit.findings.push(finding("legacy_or_malformed", error.to_string()));
                break;
            }
            None => break,
        };
        validate_event(&event, reader.symbols(), config, &mut audit.findings);
        *audit.streams.entry(event.provenance.stream.clone()).or_default() += 1;
        audit.event_count += 1;
    ...
    if audit.event_count == 0 {
        audit.findings.push(finding("empty_log", "no readable events"));
    }
    for stream in &config.required_streams {
        if !audit.streams.contains_key(stream) {
            audit.findings.push(finding("missing_stream", stream.clone()));
        }
    }
    if !audit.gaps.is_empty() {
        audit.findings.push(finding("coverage_gap", format!("{} gap(s)", audit.gaps.len())));
    }
...
fn validate_event(
    ...
    if event.venue != config.venue {
        findings.push(finding("venue_mismatch", format!("event has {:?}", event.venue)));
    }
        ...
        Some(meta) if meta.symbol_id == event.symbol => {
            if meta.venue != config.venue || meta.venue_symbol != config.symbol {
                findings.push(finding(
                    "symbol_mismatch",
                    format!("symbol id {} resolves to {:?}/{}", event.symbol.0, meta.venue, meta.venue_symbol),
                ));
            }
        }
    ...
    if event.provenance.stream.is_empty()
        || event.provenance.subscription.is_empty()
    {
        findings.push(finding("missing_provenance", "stream/subscription is not live-attributable"));
    }
    if matches!(event.body, MarketEvent::BookSnapshot { .. })
        && matches!(event.provenance.snapshot_source, SnapshotSource::None)
    {
        findings.push(finding("missing_snapshot_source", "book snapshot has no source"));
    }
}
...
fn finding(code: impl Into<String>, detail: impl Into<String>) -> AuditFinding {
    AuditFinding { code: code.into(), detail: detail.into() }
}
...
pub fn scorecard(date: impl Into<String>, entries: Vec<(Venue, String, RawLogAudit)>) -> DailyScorecard {
    ...
    DailyScorecard { date: date.into(), recordings, promotable }
}
...
mod tests {
    ...
    fn event(recv: i64) -> EventEnvelope {
        EventEnvelope::new(
            ...
            recv,
            recv as u64,
            MarketEvent::Trade { price: 1.0, qty: 1.0, side: Side::Buy, trade_id: recv as u64 },
        )
    ...
    fn fixture(events: Vec<EventEnvelope>) -> std::path::PathBuf {
        ...
        writer.write_symbols(&[mp_core::SymbolMeta::new(
            SymbolId(0), Venue::BinanceFutures, "BTCUSDT", "BTC", "USDT",
            mp_core::InstrumentKind::Perp, 0.1, 0.001, 1.0,
        )]).unwrap();
        for event in events { writer.append(&event).unwrap(); }
        writer.sync().unwrap();
        path
    }
    ...
    fn int_1_event_provenance_roundtrips() {
        ...
        let audit = audit_raw_log(&path, &AuditConfig::single(Venue::BinanceFutures, "BTCUSDT"));
    ...
    fn int_2_mixed_log_is_quarantined() {
        ...
        let audit = audit_raw_log(&path, &AuditConfig::single(Venue::BinanceFutures, "BTCUSDT"));
        assert!(audit.findings.iter().any(|finding| finding.code == "venue_mismatch"));
        let _ = std::fs::remove_file(path);
    }
    ...
    fn int_3_audit_reports_gap_and_staleness() {
        ...
        stale.body = MarketEvent::Status { kind: mp_core::StatusKind::Stale, detail: "stale".into() };
    ...
    fn int_5_scorecard_requires_every_recording_clean() {
        let clean = RawLogAudit { event_count: 1, first_recv_ts_ns: Some(1), last_recv_ts_ns: Some(1), coverage: 1.0, streams: BTreeMap::new(), gaps: vec![], stale_periods: vec![], findings: vec![] };
        let bad = RawLogAudit { findings: vec![finding("missing_stream", "book")], ..clean.clone() };
        let card = scorecard("2026-07-29", vec![
