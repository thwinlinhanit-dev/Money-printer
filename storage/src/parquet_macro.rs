//! Macro points ↔ Parquet (spec 030, MAC-6). One file per venue/symbol/date,
//! sorted by `recv_ts_ns`, zstd-compressed, with the same footer KV metadata
//! contract as trades (STO-8). Partition:
//! `cold/macro/venue=…/symbol=…/date=…/part-000.parquet` (separate stream from
//! trades, W-6). Manifest `sampled: false` — FRED daily series are
//! authoritative observations, not a throttled sample (MAC-6).
...
/// Write `MacroPoint` events (one venue/symbol/date, sorted by recv) to a
/// Parquet file with footer metadata. Non-macro events are ignored.
pub fn write_macro(
    path: &Path,
    ...
    let (mut sym, mut ven, mut ex, mut rv, mut sq, mut sid, mut val, mut date) = (
        Vec::new(),
        Vec::new(),
        Vec::new(),
        Vec::new(),
    ...
    let mut writer = ArrowWriter::try_new(file, macro_schema(), Some(props))
...
/// Read macro points Parquet back into `EventEnvelope`s (MAC-6 roundtrip).
pub fn read_macro(path: &Path) -> Result<Vec<EventEnvelope>, StorageError> {
    ...
    for batch in reader {
        ...
        let sid = col_str(&batch, 5)?;
        let val = col_f64(&batch, 6)?;
        let date = col_i64(&batch, 7)?;
        for i in 0..batch.num_rows() {
            ...
            out.push(EventEnvelope::new(
                ...
                MarketEvent::MacroPoint {
                    series_id: sid.value(i).to_owned(),
                    value: val.value(i),
                    date: date.value(i),
                },
            ));
