...
def detect_v2_signals(bars, min_streak=3):
    """Detect V2 range breakout signals with relaxed constraints."""
    signals = []
    sma_period = 50
    
    for i in range(max(sma_period, 30), len(bars) - 30):
        c = bars[i]
        ...
        # Look for range formation (15-bar window)
        range_high = max(b.high for b in bars[i:i+15])
        range_low = min(b.low for b in bars[i:i+15])
        range_width = range_high - range_low
        
        # RELAXED: Allow wider ranges (6x ATR instead of 4x)
        if range_width > 6 * atr14:
            continue
        
        # Look for breakout in next 15 bars
        for k in range(i+1, min(i+15, len(bars))):
            breakout_bar = bars[k]
            vol_ma = sum(b.volume for b in bars[max(0,k-20):k]) / min(20, k)
            
            # Breakout up
            if (breakout_bar.close > range_high + 0.10 * atr14 and  # Reduced buffer
                breakout_bar.volume > vol_ma * 1.10):  # Reduced volume requirement
                signals.append({
                    'idx': k,
            ...
            # Breakout down
            if (breakout_bar.close < range_low - 0.10 * atr14 and
                breakout_bar.volume > vol_ma * 1.10):
                signals.append({
                    'idx': k,
                    'date': breakout_bar.date,
                    'close': breakout_bar.close,
