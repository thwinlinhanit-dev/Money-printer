...
def test_res5_brief_has_all_fixed_sections():
    md = render_brief(fixture_inputs())
    for section in SECTIONS:
        assert f"## {section}" in md, f"missing section {section}"


def test_res5_missing_section_renders_no_data_not_invention():
...
def test_res6_brief_quotes_only_input_numbers():
    inputs = fixture_inputs()
    md = render_brief(inputs)
...
def test_res6_ungrounded_number_is_detected():
    inputs = fixture_inputs()
...
def test_res5_generate_or_alert_returns_p3_on_ungrounded(monkeypatch):
    inputs = fixture_inputs()
...
def test_res6_input_bundle_hash_is_deterministic():
...
def test_res6_archive_writes_reproducible_record(tmp_path):
...
def test_res7_archive_refuses_to_write_outside_briefs_dir(tmp_path):
