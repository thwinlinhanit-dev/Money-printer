...
def _write_fixture(root, with_gap: bool):
    ...
    # Normative schema: day_start_ns / day_end_ns (spec 003, Rust QualityManifest).
    manifest = {
        "day_start_ns": 0,
        "day_end_ns": DAY,
...
def test_manifest_schema_contract_rejects_old_from_to_ns_names(tmp_path):
    """Regression: old from_ns/to_ns manifest fields must raise loudly so a
    stale research fixture never silently feeds the coverage gate (blocker #2
    fix — one normative schema, fail loud on mismatch)."""
