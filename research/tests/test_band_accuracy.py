...
def report(
    *,
    ...
    return {
        "study": "whale_study",
        "git_sha": "abc1234",
        "params": {"maintenance_buffer": 0.05, "leverage_tiers": [{"leverage": 10.0, "weight": 1.0}]},
        "events": {"mark": 10, "open_interest": 5, "whale_positions": 2, "unmapped_skipped": 0},
...
def test_res_4_band_parse_report_fields():
    ...
    assert r.long.n == 1 and r.long.mean_relative_error == 0.020 and r.long.coverage == 0.900
    assert r.short.n == 1 and r.short.mean_relative_error == 0.040 and r.short.coverage == 0.800
    assert r.total.n == 2 and r.total.mean_relative_error == 0.030 and r.total.coverage == 0.850
    assert r.events == {"mark": 10, "open_interest": 5, "whale_positions": 2, "unmapped_skipped": 0}
...
def test_res_4_band_weekly_trend_weighted_merge_and_order():
    a = make_run(TS_2026_W32, long=(1, 0.020, 0.900), short=(1, 0.040, 0.800), total=(2, 0.030, 0.850))
    # Same week, heavier run: n-weighted MRE/coverage.
    b = make_run(TS_2026_W32, long=(3, 0.100, 0.600), short=(1, 0.000, 1.000), total=(4, 0.075, 0.700))
    # Earlier week sorts first (cross-year lexical == chronological).
    c = make_run(TS_2025_W31, long=(1, 0.010, 0.950), short=(0, 0.0, 0.0), total=(1, 0.010, 0.950))
...
def test_res_4_band_weekly_trend_zero_n_week():
    zero = make_run(TS_2026_W32, long=(0, 0.0, 0.0), short=(0, 0.0, 0.0), total=(0, 0.0, 0.0))
...
def test_res_4_band_job_happy_path_journals_week_and_trend(tmp_path):
    ...
    path, ran, payload = run_weekly_band_accuracy(logs, tmp_path / "out", shim, week="2026-W32")
    assert ran is True
    ...
    lines = (tmp_path / "out" / "band_accuracy.jsonl").read_text(encoding="utf-8").strip().splitlines()
    assert len(lines) == 1
    row = json.loads(lines[0])
    assert row["week"] == "2026-W32"
    assert row["n"] == 2 and row["mean_relative_error"] == 0.030 and row["coverage"] == 0.850
...
def test_res_4_band_job_explicit_week_short_circuits_refire(tmp_path):
    ...
    assert len((out / "band_accuracy.jsonl").read_text(encoding="utf-8").strip().splitlines()) == 1
...
def test_res_4_band_job_fail_closed(tmp_path):
    ...
    with pytest.raises(BandJobError):
        run_weekly_band_accuracy([tmp_path / "nope.log"], tmp_path / "out", shim, week="2026-W32")

    # Missing binary ⇒ job error.
    with pytest.raises(BandJobError):
        run_weekly_band_accuracy(logs, tmp_path / "out", tmp_path / "missing-whale", week="2026-W32")
...
def test_res_4_band_job_runs_dir_override_journals_there(tmp_path):
    ...
    path, ran, _ = run_weekly_band_accuracy(logs, tmp_path / "out", shim, week="2026-W32", runs_dir=runs)
    assert ran is True
    # The record went to the override, not the default <out-dir>/runs.
    assert (runs / "index.jsonl").read_text(encoding="utf-8").strip() == '{"study": "whale_study", "run_id": "x"}'
...
def test_res_4_band_cli_happy_and_fail_exit_codes(tmp_path):
    ...
    code = cli_main(["--log", str(log), "--whale-study", str(shim), "--out-dir", str(tmp_path / "out"), "--week", "2026-W32"])
    assert code == 0
