...
def detect_v2_signals(bars, min_streak=3):
    ...
    for i in range(max(sma_period, 30), len(bars) - 30):
        ...
        # Check if range is reasonable (within 4x ATR)
        if range_width > 4 * atr14:
            continue
        ...
        for k in range(i+1, min(i+15, len(bars))):
            ...
            # Breakout up
            if (breakout_bar.close > range_high + 0.15 * atr14 and 
                breakout_bar.volume > vol_ma * 1.15):
                signals.append({
                    'idx': k,
            ...
            # Breakout down
            if (breakout_bar.close < range_low - 0.15 * atr14 and 
                breakout_bar.volume > vol_ma * 1.15):
                signals.append({
                    'idx': k,
                    'date': breakout_bar.date,
