...
- `src/transport.rs`, `src/ws.rs` — WebSocket transport with backpressure + proactive connection rotation (`WsEndpoint::max_connection_age`, default `None`: some venues cap a connection's TOTAL lifetime regardless of traffic — Hyperliquid's edge kills every WS at ~2h48m–2h57m; when set, the transport closes voluntarily at age + ≤8% SplitMix64 jitter so sibling collectors never rotate in lockstep, turning the surprise kill into a scheduled sub-second reconnect — spec 024 amendment 2026-08-25; tests `ws_proactive_rotation_ends_connection_at_max_age` / `rotation_delay_is_age_plus_bounded_jitter`)
...
- `src/rng.rs` — workspace RNG helper; `src/staleness.rs` — stale-stream watchdog state (`stale_silences` returns each stale topic's ACTUAL observed silence, not just the static threshold — the operator-facing root-cause signal that separates a short self-healing hiccup from a real feed outage; surfaced in the COL-2 `Status::Stale` detail and parsed back out by `mp-storage::audit::parse_stale_silence_ms`)
