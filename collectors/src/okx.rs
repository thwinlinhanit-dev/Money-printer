        ...
        match channel {
            ...
            "funding-rate" => {
                for d in data {
                    ...
                    let id = self.sym(sym);
                    let rate = f64_field(d, "fundingRate")
                        .ok_or_else(|| NormError::Parse("okx funding rate".into()))?;
            ...
            "open-interest" => {
                for d in data {
                    ...
                    let id = self.sym(sym);
                    let oi = f64_field(d, "oi")
                        .ok_or_else(|| NormError::Parse("okx oi contracts".into()))?;
            ...
            "mark-price" => {
                for d in data {
                    ...
                    let id = self.sym(sym);
                    let mark = f64_field(d, "markPx")
                        .ok_or_else(|| NormError::Parse("okx mark price".into()))?;
                    let seq = self.seq();
                    out.push(EventEnvelope::new(
                        Venue::Okx,
                        id,
            ...
            "liquidation-orders" => {
                for d in data {
                    ...
                    if let Some(details) = d.get("details").and_then(|x| x.as_array()) {
                        for det in details {
                            let price = f64_field(det, "bkPx")
                                .ok_or_else(|| NormError::Parse("okx liq bkPx".into()))?;
                            let qty = f64_field(det, "sz")
                                .ok_or_else(|| NormError::Parse("okx liq sz".into()))?;
                            let side = str_field(det, "side")
                                .and_then(okx_side)
                                .ok_or_else(|| NormError::Parse("okx liq side".into()))?;
                            let seq = self.seq();
                            out.push(EventEnvelope::new(
                                Venue::Okx,
                                id,
