...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("{:<32} {:>9} {:>10} {:>19} {:>19} {:>12}", "Filename", "Size(MB)", "Events", "First Event UTC", "Last Event UTC", "Venues");
    println!("{}", "-".repeat(110));
    ...
    for path in entries {
        ...
        for ev_res in reader {
            match ev_res {
                ...
                Err(e) => {
                    println!("\n  [WARN] {} parse error at event {}: {}", filename, count, e);
                    break;
                }
            }
        }
        ...
        println!("{:<32} {:>9.2} {:>10} {:>19} {:>19} {:>12}", filename, size_mb, count, first_str, last_str, v_summary);
        if !event_types.is_empty() {
            print!("   └─ Breakdown: ");
            let ets: Vec<String> = event_types.iter().map(|(k, v)| format!("{}: {}", k, v)).collect();
            println!("{}", ets.join(" | "));
        }
    }

    println!("{}", "-".repeat(110));
    println!("TOTAL: {:.2} MB, {} events across all data files", total_bytes as f64 / (1024.0 * 1024.0), total_events);
...
fn fmt_ts(ns: i64) -> String {
    ...
    format!("{:04}-{:02}-{:02} {:02}:{:02}:{:02}.{:03}", y, m + 1, rem + 1, hours, mins, s, millis)
}
